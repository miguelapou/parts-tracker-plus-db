...
/**
 * Load an image from URL and return as base64 data URL with dimensions
 * @param {string} url - The image URL
 * @returns {Promise<{data: string, width: number, height: number}|null>} Image data with dimensions or null if failed
 */
const loadImageWithDimensions = async (url) => {
  if (!url) return null;

  try {
    // First, try to load the image to get its dimensions
    const img = new Image();
    ...
    // Draw to canvas to get base64 data
    ...
    console.error('Failed to load image via Image element, trying fetch:', error);
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        reader.onloadend = async () => {
          ...
          // Try to get dimensions from the loaded data
          const img = new Image();
          img.onload = () => {
            resolve({ data, width: img.naturalWidth, height: img.naturalHeight });
          };
          img.onerror = () => {
            // Return with default square dimensions if we can't determine size
            resolve({ data, width: 1, height: 1 });
          };
          img.src = data;
        };
...
export const generateVehicleReportPDF = async (vehicle, projects, parts, serviceEvents) => {
  ...
  const vehicleImage = await loadImageWithDimensions(vehicle.image_url);
  const doc = new jsPDF();
  ...
  // Add vehicle image thumbnail on top right if available (proportional sizing)
  const maxThumbnailSize = 35;
  if (vehicleImage && vehicleImage.data) {
    try {
      // Calculate proportional dimensions
      const aspectRatio = vehicleImage.width / vehicleImage.height;
      ...
      doc.addImage(vehicleImage.data, 'JPEG', pageWidth - margin - thumbWidth, margin, thumbWidth, thumbHeight);
    } catch (e) {
